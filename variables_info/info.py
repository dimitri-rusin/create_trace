...
def colored_bg(color):
  global log_file
  if is_colored_module_imported and log_file == sys.stderr:
...
def d(value):
  ...
  print(f'''{colored_stylize(f'at {current_time} from line {relative_path}:{line_number}: {expression}', colored_fg('green_1'))}''', file=log_file, flush=True)
  ...
  try: print(f'  {opening_name}\n{type(value)}\n  {closing_name}', file=log_file, flush=True)
  except: pass
